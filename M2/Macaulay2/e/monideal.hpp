 ...
 *
 * Declares `MonomialIdeal`, the specialised type the engine uses
 * for ideals whose generators are pure monomials. Generators are
 * stored as sparse-monomial arrays
 * `[len, v_1, e_1, v_2, e_2, ..., v_r, e_r]` with `len = 2r + 1`
 * and strictly-increasing variables (commutative case),
 * inherited from `ExponentList`. The combinatorial structure
 * lets every operation skip coefficient arithmetic and monomial-
 * ordering work entirely: intersection, sum, quotient,
 * saturation, radical, Borel closure, monomial primary
 * decomposition, and divisibility / membership / containment
 * tests are all done by walking exponent vectors directly.
 *
 * The same shape is the natural consumer of `hilb.hpp`'s
 * Hilbert-series recursion and of any initial-ideal path
 * descended from a Groebner basis. Allocation backers include
 * `std::vector<int>`, preallocated `Range` views, and
 * `MemoryBlock` arenas (`MemoryBlock.hpp`) for transient
 * inner-loop monomials.
 *
